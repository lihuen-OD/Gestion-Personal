...
export const doubleRuleSchema = z.object({ name: z.string().trim().min(2).max(120), recurrenceType: z.enum(["FECHA", "RANGO", "SEMANAL"]), fromDate: z.coerce.date(), toDate: z.coerce.date().optional().nullable(), weekdays: z.array(z.number().int().min(0).max(6)).max(7).default([]), multiplier: z.coerce.number().min(1).max(5).default(2), employeeIds: z.array(z.string().uuid()).min(1).max(1000), reason: z.string().trim().min(2).max(600), status: z.enum(["ACTIVO", "INACTIVO"]).default("ACTIVO") });
export const updateDoubleRuleSchema = doubleRuleSchema.partial().refine((value) => Object.keys(value).length > 0, { message: "Indicá al menos un dato para actualizar" });
