...
export function NoveltyTypeDetailPage() {
  ...
  return <>
    <div className="detail-hero catalog-hero"><Link to="/configuracion/tipos-novedades" className="back-link">← Volver a tipos</Link><div><div className="avatar">{item.name.slice(0, 2).toUpperCase()}</div><div><p className="eyebrow">{item.code} · {item.kind}</p><h1>{item.name}</h1><p>{item.description}</p></div></div><div className="hero-actions"><span className={item.status === "ACTIVO" ? "badge success" : "badge neutral"}>{item.status}</span><button className="table-icon-action" title={item.status === "ACTIVO" ? "Inactivar" : "Activar"} aria-label={item.status === "ACTIVO" ? "Inactivar" : "Activar"} onClick={toggle}><Power size={14} /><span>{item.status === "ACTIVO" ? "Inactivar" : "Activar"}</span></button><button className="table-icon-action danger-link" title="Ocultar" aria-label="Ocultar" onClick={() => { if (confirm("No se elimina para conservar trazabilidad. Se va a inactivar el tipo.")) toggle(); }}><Trash2 size={14} /><span>Ocultar</span></button></div></div>
