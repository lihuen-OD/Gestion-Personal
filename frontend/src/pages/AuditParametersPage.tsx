...
export function AuditParametersPage() {
  ...
  const summary = useMemo(() => [["Parametros", all.length], ["Criticos", all.filter((item) => item.severity === "CRITICO").length], ["Con motivo", all.filter((item) => item.requiresReason).length], ["Notifican", all.filter((item) => item.notification.enabled).length]] as const, [all]);
