...
export function PuestoHistoryTab({ position }: { position: Position }) {
  ...
  return <div className="timeline">{rows.map((row) => <div key={row.id}><i /><b>{auditActionLabel(row.action)}</b><span>{new Date(row.createdAt).toLocaleString("es-AR")} · {row.createdByUserName}</span><p>{row.description}{row.oldValue || row.newValue ? ` · ${row.oldValue || "-"} -> ${row.newValue || "-"}` : ""}</p></div>)}</div>;
}
