...
export function PuestoEvaluationCriteriaTab({ position, setPosition, disabled = false }: { position: Position; setPosition: (position: Position) => void; disabled?: boolean }) {
  ...
  return <div className="position-list-editor">
    <div className="form-actions inline-actions">{!disabled && <Button type="button" variant="primary" onClick={() => setPosition({ ...position, evaluationCriteria: addCriterion(position.evaluationCriteria) })}><Plus size={15} /> Agregar criterio</Button>}</div>
