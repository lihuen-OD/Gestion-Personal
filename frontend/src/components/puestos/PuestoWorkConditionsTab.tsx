...
import { modalityOptions, PuestoField, PuestoSelect, PuestoTextarea, workModalityLabels } from "./PuestoFields";
...
export function PuestoWorkConditionsTab({ position, setPosition, disabled = false }: { position: Position; setPosition: (position: Position) => void; disabled?: boolean }) {
  ...
  return <div className="form-grid">
    <PuestoSelect label="Modalidad" value={position.workConditions.modality} onChange={(value) => set({ modality: value as WorkModality })} options={modalityOptions} labels={workModalityLabels} disabled={disabled} />
