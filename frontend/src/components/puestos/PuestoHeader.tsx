...
export function PuestoHeader({ position, assignedCount, canEdit, onRemove, onToggleStatus }: { position: Position; assignedCount: number; canEdit: boolean; onRemove: () => void; onToggleStatus: () => void }) {
  return <div className="detail-hero puesto-hero">
    ...
    <div><div className="avatar">{position.name.slice(0, 2).toUpperCase()}</div><div><p className="eyebrow">PUESTO {position.code || "SIN CODIGO"}</p><h1>{position.name}</h1><p>{position.derivedAreaName || "Sin area"} · {position.derivedSectorName || "Sin sector"} · Actualizado {formatCalendarDate(position.lastUpdatedAt)}</p></div></div>
