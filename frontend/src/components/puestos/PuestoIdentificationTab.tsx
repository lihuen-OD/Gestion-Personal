...
export function PuestoIdentificationTab({ position, setPosition, disabled = false }: { position: Position; setPosition: (position: Position) => void; disabled?: boolean }) {
  ...
  const options = orgStructureMockService.getOptions();
  return <div className="form-grid">
    <PuestoField label="Nombre del puesto *" value={position.name} onChange={(value) => set("name", value)} disabled={disabled} />
    <PuestoSelect label="Unidad de negocio" value={position.businessUnitName || ""} onChange={(value) => set("businessUnitName", value)} options={["", ...options.businessUnits]} disabled={disabled} />
    <PuestoSelect label="Establecimiento" value={position.establishmentName || ""} onChange={(value) => set("establishmentName", value)} options={["", ...options.establishments]} disabled={disabled} />
    <PuestoSelect label="Area / Departamento *" value={position.areaDepartment} onChange={(value) => set("areaDepartment", value)} options={["", ...options.areas]} disabled={disabled} />
    <PuestoSelect label="Sector *" value={position.sector} onChange={(value) => set("sector", value)} options={["", ...options.sectors]} disabled={disabled} />
