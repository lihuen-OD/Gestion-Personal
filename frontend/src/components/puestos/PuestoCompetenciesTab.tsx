...
export function PuestoCompetenciesTab({ position, setPosition, disabled = false }: { position: Position; setPosition: (position: Position) => void; disabled?: boolean }) {
  ...
  return <div className="position-list-editor">
    <div className="form-actions inline-actions">{!disabled && <Button type="button" variant="primary" onClick={() => setPosition({ ...position, competencies: addCompetency(position.competencies) })}><Plus size={15} /> Agregar competencia</Button>}</div>
