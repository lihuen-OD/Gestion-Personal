...
export function PuestoResponsibilitiesTab({ position, setPosition, disabled = false }: { position: Position; setPosition: (position: Position) => void; disabled?: boolean }) {
  ...
  return <div className="position-list-editor">
    <div className="form-actions inline-actions">{!disabled && <Button type="button" variant="primary" onClick={() => setPosition({ ...position, responsibilities: addResponsibility(position.responsibilities) })}><Plus size={15} /> Agregar responsabilidad</Button>}</div>
