...
export function OrgChartLegend({ categories }: { categories: OrgCategory[] }) {
  ...
  return <div className="org-legend">{groups.map((category) => <span key={category.group}><i style={{ background: category.backgroundColor, borderColor: category.nodeColor }} />{category.label}</span>)}</div>;
}
