...
## 18. Etapa 9G — Dashboard, reportes y cargas agregadas

Fecha: 2026-08-27. Última etapa de la serie de performance. Sin rediseño de Dashboard, sin KPIs nuevos, sin cambiar cálculo de métricas, sin cambiar exportaciones funcionalmente, sin paginar Cierres, sin schema/migraciones, sin librerías nuevas — no se tocó ningún archivo del fichador (ver §18.7, es justamente donde apareció el único hallazgo real que se decidió no corregir). Conceptos Horarios/Horas Especiales sólo se verificaron (ninguno mezcla su cálculo con las métricas de dashboard — ver §18.1).

### 18.1 Diagnóstico de Dashboard

**Endpoint y agregación** — confirmado leyendo el código actual, no sólo citando 9A: `DashboardPage.tsx` llama exactamente 2 endpoints al entrar, en `Promise.all`: `GET /dashboard/metrics` (siempre) y `GET /audit?take=5` (sólo Nivel 1/RRHH, para "Actividad reciente" — no es un duplicado, es un dato distinto que el propio endpoint agregado no incluye a propósito, para no acoplar el módulo `dashboard` a `audit`). `dashboard.service.ts:calculateMetrics` sigue siendo un único `Promise.all` de 15 queries — el patrón de endpoint agregado que 9A ya había señalado como el correcto, confirmado sin cambios.

**Cache y TTL** — doble cache de 30s confirmado sin cambios: backend (`dashboard.cache.ts`, `dashboardMetricsCache`, keyed por período+usuario+rol+companyId+sectorId) + frontend (`dashboardMetricsApiService.ts`, `cachePolicies.dashboardMetrics`, mismo TTL de 30s, `sensitive:true` así que no se persiste en IndexedDB). Redundante pero no incorrecto — documentado en 9A §4.9, se re-confirma acá y se decide **no tocar** (ver criterio en §18.4).

**Invalidación de las 4 tablas fuente** (Employee, TimeEntry, Novelty, EmployeeDocument) — ver el diagnóstico completo en §18.4.

**Refresh y blanqueo de pantalla** — `DashboardPage.tsx`/`ReportsPage.tsx` blanquean toda la pantalla (`status==="loading"` reemplaza todo por un `LoadingState`) en cada corrida de su efecto de carga. Se investigó si esto es un bug real (criterio C) y **se concluyó que no**: ninguna de las 2 pantallas tiene ningún filtro interactivo (no hay selector de período, empresa ni sector visible en el JSX de ninguna de las dos) — el único disparador del efecto después del montaje inicial es el botón "Reintentar" del `ErrorState`, que sólo es alcanzable **desde** el estado de error (nunca hay datos buenos en pantalla en ese momento para "perder"). A diferencia de `HoursPage`/`FinnegansExportPage`, acá no existe la secuencia "datos buenos visibles → cambia un filtro → se blanquea" porque no hay ningún filtro que cambiar. Aplicar el mismo guard `if (!data) setStatus("loading")` no habría cambiado ningún comportamiento observable — se documenta la razón en vez de aplicar un cambio sin efecto medible (regla "medir antes de reescribir" de 9A/9F).

**Horas reales vs. liquidables** — confirmado leyendo `dashboard.repository.ts:sumLoadedHours`: agrega `TimeEntry.hours` filtrado por `hourConcept.systemRole === "NORMAL_BASE"` — el mismo pass-through puro ya confirmado en 8F/9A (nunca multiplica por `appliedMultiplier`, nunca mezcla con `HourConceptBreakdown`). Horas Especiales no aparece en ningún KPI del dashboard como valor separado — el dashboard no muestra "horas liquidables" en absoluto, sólo horas reales cargadas (`loadedHours`) y cobertura (`loadCoverage`), consistente con la matriz de clasificación de 9A §2.2 (categoría D: nunca mostrar un valor liquidable sin separarlo claramente — acá directamente no se muestra, así que no hay riesgo de mezcla).

**Hallazgos menores, no corregidos (fuera del mandato de performance de esta etapa)**: el botón "Exportar resumen" de `DashboardPage.tsx` no tiene ningún `onClick` — no hace nada al presionarlo. No es un bug de carga de datos/performance (nunca dispara ninguna llamada, ni de más ni de menos) — es una funcionalidad faltante, y el pedido de esta etapa prohíbe explícitamente "cambiar exportaciones funcionalmente"/"agregar nuevos KPIs". Se documenta para una etapa de UX/funcionalidad futura, no se tocó. También: `fallbackScope` (`useMemo` en `DashboardPage.tsx:77-80`) se calcula pero nunca se usa — `dashboardMetricsApiService.getMetrics(_scope?: unknown, ...)` ignora ese primer parámetro por completo (nombrado `_scope`, código muerto de una versión anterior con filtrado client-side). No genera ningún costo de red ni de render medible (un `useMemo` barato, descartado) — se deja sin tocar, es un nit de limpieza de código sin relación con performance.

### 18.2 Diagnóstico de reportes/exportaciones

- **`ReportsPage.tsx`** — mismo patrón que Dashboard (1 endpoint agregado, sin filtros interactivos, blanqueo sólo alcanzable vía retry-tras-error). Mismo diagnóstico, mismo resultado: no es un bug, no se tocó.
- **`FinnegansExportPage.tsx`** — **acá sí había un bug real** (ver §18.5): tiene un selector de período **en vivo**, y el fetch de la vista previa (`finnegansExportApiService.getNoveltyRows(period)`) blanqueaba la tabla completa en cada cambio de período, con filas ya cargadas — el mismo patrón exacto ya corregido en 6 pantallas distintas a lo largo de 9B/9C/9E. Corregido con el mismo guard, ver §18.5.
- **Exportación no se dispara al montar la pantalla**: confirmado — `getNoveltyRows(period)` alimenta únicamente la **tabla de vista previa** (ya acotada por período, sin ningún límite tipo `take:5000`); el botón "Exportar Excel Finnegans" arma el `.xlsx` **en el cliente** (librería `xlsx`, ya presente, `import()` dinámico) a partir de las filas que ya están en pantalla — no dispara ninguna llamada de red adicional. Igual patrón ya confirmado para `HoursPage.exportHours()` en 9A/9F (fetch bajo demanda, sólo al hacer click) — acá el fetch ocurre antes (para la vista previa), pero el volumen es el mismo (acotado por período), así que no hay datos masivos cargados de más en ningún caso.
- **`timeEntries.repository.ts:681` (`take:5000`, `findForExport`)** — re-confirmado sin cambios desde 9A: acotado por período+empleado+estado, deliberado, documentado en comentarios de etapas anteriores (8F/6M). No se tocó.

### 18.3 Diagnóstico de cierres mensuales

Re-confirmado, sin cambios desde 9C/9E: `MonthlyTimeClosure=0` filas hoy (lectura de sólo lectura contra la base conectada, repetida en esta etapa), `TimeCorrectionRequest=0`, `Employee=12`. La razón documentada en 9E §16.5 para no paginar sigue vigente sin cambios: es una pantalla de selección múltiple para acciones en lote ("Seleccionar pendientes" → aprobar/enviar en lote), y paginar sin definir primero el alcance de "seleccionar todos" (página visible vs. todo el período) sería un cambio de comportamiento funcional, no una mejora de performance transparente — coincide exactamente con la instrucción explícita de esta etapa ("No paginar cierres en esta etapa salvo que exista una solución funcional clara y testeable" — no apareció ninguna). El guard de refresh silencioso de 9B (`hasLoadedDataRef`) se confirmó intacto, sin tocar. El único riesgo de cache stale en una acción crítica de esta pantalla (aprobar una corrección post-cierre, que reescribe `TimeEntry.hours`) **sí tenía un hueco real** — corregido en §18.5.

### 18.4 Invalidaciones revisadas

Se investigó exhaustivamente (grep de todo `backend/src` para `.employee.`/`.timeEntry.`/`.novelty.`/`.employeeDocument.` con `.create/.update/.delete/.upsert/.updateMany/.deleteMany/.createMany`, siguiendo cada write site hasta su función de servicio) si el cache de dashboard queda stale después de: crear/editar/borrar horas, aprobar/rechazar/devolver horas, correcciones post-cierre, novedades, documentos, cambios de empleados, cierres mensuales, horas especiales, conceptos horarios.

**Hallazgo de diseño confirmado**: `audit.service.ts:register()` es un punto de invalidación **centralizado** — cada vez que se llama, limpia incondicionalmente tanto `clearAuditListCache()` como `clearDashboardMetricsCache()`. Como la convención ya establecida en este proyecto (`CLAUDE.md`: "No change to employees, time-entries, novelties or auth business logic is complete without an accompanying test", y en la práctica toda mutación relevante llama `auditService.register(...)` por trazabilidad) hace que casi todas las escrituras pasen por ahí, la invalidación de dashboard "viaja gratis" sobre la trazabilidad de auditoría — no hace falta invalidar el cache de dashboard a mano en cada mutador nuevo del backend, alcanza con no olvidar el registro de auditoría (que de por sí ya es obligatorio).

**Resultado, modelo por modelo**:
- **Employee** (total/active/inactive/exits/transported/averageAge/averageTenure/headcountByCompany/headcountBySector/missingResponsible): **100% cubierto** — create/update/updateContact/upsertAddress/upsertTransport/replaceAssignments/createLaborMovement/syncLaborStatuses, todos llaman `auditService.register()`.
- **Novelty** (absenceDays/absenceRate/pendingNovelties): **100% cubierto** — create/approve/reject/delete (incluida la aprobación en lote, un `register()` por ítem) llaman `auditService.register()`.
- **EmployeeDocument** (expiredDocuments/expiringDocuments): **100% cubierto** — el único write path que existe en todo el backend es la creación del documento (`employees.service.ts:createDocument`), que sí llama `auditService.register()`. Se confirmó explícitamente que **no existe ningún job/cron que reevalúe el estado VIGENTE/POR_VENCER/VENCIDO por fecha** — el estado se fija una sola vez, al cargar el documento, y nunca se vuelve a tocar. No es un hueco de invalidación (no hay ninguna escritura que se esté perdiendo) — es una pregunta de "estos contadores reflejan una foto vieja de vencimientos" que excede el alcance de esta etapa (sería un cambio de lógica de negocio: agregar un sweep de vencimientos, no una corrección de cache).
- **TimeEntry** (loadedHours/loadCoverage/pendingLoads/reviewLoads): **cubierto para carga manual, aprobación/rechazo/devolución, y corrección post-cierre en backend** (todas llaman `auditService.register()`, incluida `approveCorrection`). **Hueco real encontrado en 2 caminos, ambos dentro del fichador**: ver §18.7 — no se corrigieron, por la regla explícita "No tocar fichador".
- **Frontend**: `noveltyApiService.ts`/`employeeApiService.ts`/`timeEntryApiService.ts`/`documentApiService.ts`/`orgStructureApiService.ts` ya invalidan la familia `"dashboard"` del lado del cliente en sus mutaciones relevantes (confirmado leyendo cada uno, no asumido) — **excepto `workforceApiService.ts`, que no invalidaba ninguna familia de cache en absoluto** (ni siquiera importaba `invalidateCacheFamily`). La única mutación de ese archivo que afecta una métrica de dashboard es `reviewCorrection(id, "approve")` (reescribe `TimeEntry.hours` en el backend) — corregido, ver §18.5.

**Ningún hallazgo de esta sección implica staleness más allá de la ventana de 30s del TTL** — incluso en el peor caso de un hueco de invalidación, el dato se autocorrige solo al expirar el cache. Se documentan igual porque el pedido de esta etapa lo exige explícitamente ("revisar si el cache se invalida... después de" cada una de esas 8 acciones) y porque, para `TimeEntry` específicamente, el hueco es de alta frecuencia (cada fichada) aunque de bajo impacto (30s).

### 18.5 Qué se modificó

**`frontend/src/services/api/workforceApiService.ts`** — `reviewCorrection(id, decision, note)` ahora invalida la familia de cache `"dashboard"` del lado del frontend, únicamente cuando `decision === "approve"` (rechazar una corrección no toca `TimeEntry`, así que no hace falta invalidar nada ahí — "no invalidar todo por las dudas"). El backend ya limpiaba su propio cache vía `auditService.register()`; esto cierra la capa de cache del frontend, que quedaba sirviendo el valor de "Horas cargadas" viejo hasta que su propio TTL de 30s expirara solo. Sin cambio de contrato (misma respuesta, mismo endpoint). Tests: 3 nuevos en `workforceApiService.test.ts` (invalida al aprobar, NO invalida al rechazar, no cambia la respuesta).

**`frontend/src/pages/FinnegansExportPage.tsx`** — mismo guard `if (!rows.length) setStatus("loading")` ya usado en 6 pantallas de 9B/9C/9E, aplicado al único efecto de carga (dependencia `[period, retry]`). Cambiar de período con filas ya cargadas ya no blanquea la vista previa. Tests: 3 nuevos en `FinnegansExportPage.test.tsx` (archivo nuevo — loading inicial, no blanquea al cambiar de período, la exportación no dispara ninguna llamada al montar).

### 18.6 Qué quedó documentado como correcto (sin tocar)

- **Dashboard**: endpoint agregado, 15 queries en un único `Promise.all`, doble cache de 30s (redundante pero inofensivo), sin filtros interactivos por lo que el blanqueo en loading nunca pisa datos buenos — el patrón de referencia que 9A ya había identificado, reconfirmado.
- **Reportes** (`ReportsPage.tsx`): mismo patrón que Dashboard, mismas conclusiones.
- **Exportación Finnegans**: fetch de vista previa acotado por período, exportación 100% client-side sin llamada adicional.
- **Export de nómina** (`findForExport`, `take:5000`): deliberado, acotado, ya confirmado en 8F/9A, re-confirmado sin cambios.
- **Cierres mensuales**: no paginar sigue siendo la decisión correcta (razón funcional, no de volumen); el guard de 9B sigue intacto.
- **Invalidación de dashboard para Employee/Novelty/EmployeeDocument**: comprobadamente completa (100% de los write paths encontrados llaman `auditService.register()`), no se tocó nada porque no hacía falta.
- **Horas reales vs. liquidables en el dashboard**: pass-through puro confirmado, sin mezcla con Horas Especiales ni Conceptos Horarios (el dashboard directamente no muestra ningún valor liquidable).

### 18.7 Qué quedó pendiente — el hallazgo real que no se corrigió

Se encontraron 2 caminos de escritura a `TimeEntry` sin ningún `auditService.register()` (y por lo tanto sin invalidación de dashboard, acotada de todos modos a 30s por el TTL) — **ambos dentro del fichador, así que no se tocaron**, por la regla explícita e incondicional "No tocar fichador" de esta etapa (a diferencia de Horas Especiales/Conceptos Horarios/Carga horaria, que sí tenían una excepción de "salvo verificación"):

1. **`timeEntries.service.ts` — `clockInResolved()`/`clockOutResolved()`** (el fichador por DNI, sin foto — `timeEntries.controller.ts`: `clockIn`/`clockInByEmployee`/`clockOut`/`clockOutByEmployee`): a diferencia de su hermano con foto (`clockPhotoPunch`, que sí llama al helper `scheduleClockAudit()`), este camino nunca registra auditoría. Es probablemente el write path de mayor frecuencia de todo el sistema (cada fichada de cada persona, todos los días).
2. **`backend/src/modules/time-entries/clockPunchMaintenance.ts` — `maintainClockPunchAttempts()`**, un job en `setInterval` (60s por defecto, registrado en `server.ts`) que llama `expireOpenWorkShifts()` y crea filas de `TimeEntry` para jornadas abiertas olvidadas — tampoco registra auditoría en ningún punto de la cadena.

**Por qué no se corrigió pese a ser un hallazgo real y evidenciado**: ambos caminos son parte del núcleo de escritura del fichador — tocar `timeEntries.service.ts`/`clockPunchMaintenance.ts` para agregar una llamada de auditoría, aunque no cambie ningún timestamp ni cálculo de horas, sigue siendo modificar el camino de escritura más sensible de la aplicación (el mismo que 8B/9A establecieron como "sin optimistic update, sin cache, máxima trazabilidad"). El pedido de esta etapa es explícito y sin excepciones para este módulo. Se documenta con la máxima precisión posible (archivo:línea de cada gap, cuál es el patrón correcto ya existente al lado — `scheduleClockAudit()` — que podría replicarse) para que una etapa futura, dedicada específicamente al fichador y con autorización explícita para tocarlo, lo resuelva sin tener que re-investigar desde cero. El impacto práctico hoy es acotado: el dato real en `TimeEntry` se escribe correctamente siempre (no hay ningún bug de horas incorrectas); el único efecto es que el dashboard puede tardar hasta 30s (su TTL) en reflejar una fichada reciente en vez de invalidarse al instante — dentro de lo que 9A ya clasificó como aceptable para una pantalla de categoría E (agregados, no crítica).

### 18.8 Tests agregados

Frontend: `workforceApiService.test.ts` (+3 tests), `FinnegansExportPage.test.tsx` (+3, archivo nuevo). Total frontend: 375 tests (369 + 6). Backend: sin cambios de código, sin tests nuevos — 709 tests siguen pasando igual.

### 18.9 Validaciones ejecutadas

Frontend: `npx tsc -b` ✅, `npx vitest run` ✅ 375/375, `npm run build` ✅.
Backend (sin cambios, corrido igual por seguridad): `npx prisma validate` ✅, `npx prisma generate` ✅, `npx prisma migrate status` ✅ (45 migraciones, sin cambios), `npm run typecheck` ✅, `npx vitest run` ✅ 709/709, `npm run build` ✅.
General: `git diff --check` sin errores de espacios en blanco.

### 18.10 Reglas futuras para dashboards/reportes

- Antes de agregar invalidación manual de cache en un mutador nuevo, confirmar primero si ya pasa por un punto de invalidación centralizado (acá, `auditService.register()`) — puede que ya esté cubierto sin necesidad de tocar nada.
- Un cache con TTL corto (30s) acota el impacto de cualquier hueco de invalidación a "hasta 30s de atraso" — eso cambia la severidad de "bug" a "detalle de latencia", pero sigue valiendo la pena documentar el hueco si el write path es de alta frecuencia (aunque no se corrija, como en §18.7).
- Antes de "corregir" un blanqueo de pantalla en loading, confirmar que existe al menos un camino real (filtro interactivo, botón de refresh) que dispare un refetch con datos buenos ya en pantalla — si el único disparador post-montaje es un retry alcanzable sólo desde el propio estado de error, no hay nada que blanquear y el guard no cambiaría ningún comportamiento observable.
- Un endpoint agregado (`Promise.all` de N queries en el backend) sigue siendo la referencia correcta para cualquier pantalla de KPIs nueva — no crear N llamadas del lado del frontend cuando el backend puede resolverlo en una.
- Ningún cambio de cache/invalidación debe tocar el camino de escritura del fichador sin autorización explícita y dedicada a ese módulo, sin importar cuán inofensivo parezca el cambio — la regla "no tocar fichador" no tiene excepción de "salvo bug real" en esta serie, a propósito.
