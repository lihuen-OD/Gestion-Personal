...
**Ejemplo real del proyecto**: `GET /dashboard/metrics` — un único endpoint, cacheado 30s en backend (`dashboard.cache.ts`) y otros 30s en frontend (`dashboardMetricsApiService.ts`). El patrón de "1 endpoint agregado" fue confirmado correcto en 9A y re-confirmado sin cambios en 9G. **Actualizado en 14E.1**: `dashboard.service.ts:calculateMetrics` hacía `Promise.all` de 15 queries sin límite de concurrencia — bajo carga real (journeys de Legajos 14D.1-14D.7, que aterrizan en esta pantalla tras el login) esto saturaba intermitentemente el pool de conexiones de Neon (`P1017`/"Server has closed the connection", con distintos modelos fallando en distintas corridas — señal de contención, no de un bug puntual). 9A/9G nunca probaron este endpoint bajo ese nivel de concurrencia real, así que el hallazgo no contradice esas revisiones, las extiende con evidencia nueva. Desde 14E.1, las mismas 14 queries (`countTotal`+`countActive` colapsadas en 1 `groupBy`) corren en lotes de máximo 5 concurrentes (`runInBatches`, `backend/src/shared/prisma/runInBatches.ts`) — sigue siendo **1 endpoint agregado con `Promise.all` interno** (el principio de esta sección no cambió), sólo que acotado en lotes en vez de sin límite. Ver `docs/decisions/DASHBOARD_METRICS_PERFORMANCE_14E1.md`.
...
- **Un `Promise.all` de más de ~10 queries Prisma independientes debe correr en lotes de concurrencia acotada** (`runInBatches`, `backend/src/shared/prisma/runInBatches.ts`; tamaño de lote a definir por el caso, 5 es el valor ya en uso en `dashboard.service.ts`), no sin límite — un fan-out sin acotar puede saturar el pool de conexiones bajo carga real, aunque funcione bien en pruebas aisladas de baja concurrencia (ver el caso real de `dashboard/metrics`, Etapa 14E.1, `docs/decisions/DASHBOARD_METRICS_PERFORMANCE_14E1.md`). Esto no contradice la regla anterior (seguir agregando en 1 endpoint) — sólo acota cuánta concurrencia real se le pide a la base en ese único endpoint.
